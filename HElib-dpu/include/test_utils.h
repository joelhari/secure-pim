...
#define EQUALS_LONG_ARRAY(xs, ys, size)                                                                                                                                     \
    {                                                                                                                                                                       \
        for (size_t i = 0; i < size; i++)                                                                                                                                   \
        {                                                                                                                                                                   \
            if (xs[i] != ys[i])                                                                                                                                             \
            {                                                                                                                                                               \
                std::cout << "    ERROR: " << __FUNCTION__ << " failed on line " << __LINE__ << ": at array index " << i << ":  " << xs[i] << " != " << ys[i] << std::endl; \
            }                                                                                                                                                               \
        }                                                                                                                                                                   \
    }
...
void copy_long_array(long *source, long *target, size_t size)
{
    for (size_t i = 0; i < size; i++)
    {
        target[i] = source[i];
