...
int main(void)
{
    ...
    if (per_block_tasklets > 0) {
        ...
        {
            do_butterfly0 = DPU_DO_FWD_BUTTERFLY0 == 1 && idx == 0 ? 1 : 0;
        }

        fwd_butterfly_loop(mram_read_start, per_tasklet_size, do_butterfly0);
    }
    else
    {
        for (uint8_t i = 0; i < per_tasklet_blocks; i++)
        {
            ...
            fwd_butterfly_loop(mram_read_start, per_tasklet_size, 1);
        }
    }
