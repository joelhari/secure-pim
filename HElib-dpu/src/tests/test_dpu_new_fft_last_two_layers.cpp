...
void test_dpu_new_fft_last_two_layers(void)
{
    ...
    // perform helib new_fft_last_two_layers
    helib::PGFFT::test_new_fft_last_two_layers(xp, TEST_BLOCKS);

    // perform dpu new_fft_last_two_layers
#ifdef USE_MULTI_TASKLETS
    dpu_new_fft_last_two_layers_2(dpu_xp, TEST_BLOCKS);
#else
    dpu_new_fft_last_two_layers(dpu_xp, TEST_BLOCKS);
#endif
...
int main(void)
{
    ...
    for (size_t i = 0; i < TEST_RUNS; i++)
    {
        ...
        std::cout.flush();
        test_dpu_new_fft_last_two_layers();
    }
