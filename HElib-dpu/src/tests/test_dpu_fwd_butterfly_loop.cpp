...
void test_dpu_fwd_butterfly_loop(void)
{
    ...
    // perform helib fwd_butterfly_loop
    helib::PGFFT::test_fwd_butterfly_loop(TEST_SIZE, xp0, xp1, wtab);

    // perform dpu fwd_butterfly_loop
#ifdef USE_MULTI_TASKLETS
    dpu_fwd_butterfly_loop_2(TEST_SIZE, dpu_xp0, dpu_xp1, dpu_wtab);
#else
    dpu_fwd_butterfly_loop(TEST_SIZE, dpu_xp0, dpu_xp1, dpu_wtab);
#endif
