...
void test_dpu_new_ifft_layer(long blocks, long size)
{
    ...
    // perform helib new_ifft_layer
    helib::PGFFT::test_new_ifft_layer(xp, blocks, size, wtab);

    // perform dpu new_ifft_layer
#ifdef USE_MULTI_TASKLETS
    dpu_new_ifft_layer_2(dpu_xp, blocks, size, wtab);
#else
    dpu_new_ifft_layer(dpu_xp, blocks, size, wtab);
#endif
...
int main(void)
{
    ...
    while (size >= 8)
    {
        ...
        test_dpu_new_ifft_layer(blocks, size);
