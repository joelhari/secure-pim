...
// #define ENABLE_DPU_LOG 1

#ifdef ENABLE_DPU_LOG
#define LOG_FUNCTION() Info(std::string("LOG FUNCTION: ") + __func__);
#else
#define LOG_FUNCTION()
#endif
...
public:
  ...
  friend inline void Info(const char* msg);
  friend inline void Debug(const char* msg);
};
...
{
  *helog.logStream_p << timestamp() << " INFO: " << msg << std::endl;
}
...
{
  *helog.logStream_p << timestamp() << " DEBUG: " << msg << std::endl;
}

inline void Debug(const std::string& msg) { Debug(msg.c_str()); }
